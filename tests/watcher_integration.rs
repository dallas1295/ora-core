...
fn watcher_full_integration_test() -> Result<(), OraError> {
    ...
    let docs_dir = dirs::document_dir().ok_or_else(|| OraError::Other("Cannot find Documents directory".to_string()))?;
    ...
    assert!(indexed_note.is_some(), "New file should be indexed by watcher");
    ...
    assert!(updated_note.is_some(), "Modified file should still be indexed");
    ...
    println!("Debug: Does content contain 'Modified content'? {}", note.content.contains("Modified content"));
    ...
    assert!(nested_indexed.is_some(), "Nested directory file should be indexed");
    ...
    fs::write(shelf_path.join("Markdown File.md"), "# Markdown File\nShould be indexed")?;
    fs::write(shelf_path.join(".hidden.md"), "# Hidden Markdown\nShould not be indexed")?;
    thread::sleep(Duration::from_millis(500));
    
    assert!(index.get_by_path(&shelf_path.join("Markdown File.md"))?.is_some(), "Visible markdown should be indexed");
    assert!(index.get_by_path(&shelf_path.join("test.txt"))?.is_none(), "Text file should not be indexed");
    assert!(index.get_by_path(&shelf_path.join("test.json"))?.is_none(), "JSON file should not be indexed");
    assert!(index.get_by_path(&shelf_path.join(".hidden.md"))?.is_none(), "Hidden markdown should not be indexed");
    ...
    assert!(index.get_by_path(&deletable_path)?.is_some(), "File should be indexed before deletion");
    ...
    assert!(index.get_by_path(&deletable_path)?.is_none(), "Deleted file should be removed from index");
    ...
    for i in 0..5 {
        fs::write(&debounce_path, &format!("# Rapid Change {}\nContent version {}", i, i))?;
    ...
    assert!(debounced_note.is_some(), "File should be indexed after debouncing");
    let content = debounced_note.unwrap().content;
    assert!(content.contains("Rapid Change 4"), "Should contain final version");
    assert!(!content.contains("Rapid Change 0"), "Should not contain first rapid change");
    ...
    for i in 0..3 {
        ...
        let handle = thread::spawn(move || {
            ...
            fs::write(&note_path, &format!("# Concurrent Note {}\nContent for note {}", i, i)).unwrap();
        });
        handles.push(handle);
    }
    ...
    for i in 0..3 {
        ...
        assert!(index.get_by_path(&note_path)?.is_some(), "Concurrent file {} should be indexed", i);
    }
